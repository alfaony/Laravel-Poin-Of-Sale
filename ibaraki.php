<?php
				class pesan extends Line_Apps{
					...
					function on_follow()
					{
						...
						// Alamat
						$this->session->set('setalamat',null);
						$this->session->set('nama',null);
						$this->session->set('alamat',null);
						$this->session->set('no_hp',null);
								...
								switch($this->session->get('barista'))
								{
									...
									case "kerja":
										if($text  == 'tutup')
										{
											...
											$tutup1  = $db->query("UPDATE profiles SET session_data = 'null' WHERE status ='service' and WHERE status ='operator'");
									...
									switch ($data->status) 
                                    {
										case 'service':														
											//menu
                                                    ...
                                                    if($this->session->get('pilih') == null)
                                                    {
                                                    ...
                                                    switch ($this->session->get('pilih'))
                                                    {
                                                            ...
                                                            switch ($text){
                                                                case 'selesai':
                                                                    ...
                                                                    $db = new Database();
                                                                            ...
                                                                            $m[] = "Terimakasih.Password wifi : ibarakikoffie229";
                                                                ...
                                                                default:
                                                                    ...
                                                                    if($data->ketersediaan == 'kosong')
                                                                        {
                                                                            ...
                                                                            "altText"=> "Flex Message",
                                                                            "contents" => array("type"=>"bubble","body"=>array("type"=> "box","layout"=> "vertical","spacing"=> "sm","contents"=> array(
                                                                                    array("type"=> "text","text"=> "{$data->name}  sedang Tidak tersedia","size"=> "sm","weight"=> "bold","wrap"=> true))),
                                                                                'footer'=>array("type"=> "box","layout"=> "vertical","spacing"=> "sm","contents"=> array(
                                                                                    array(
                                                                    ...
                                                                    if($data == null)
                                                                    {
                                                                        if($data->ketersediaan == 'kosong')
                                                                        {
                                                                            ...
                                                                            "altText"=> "Flex Message",
                                                                            "contents" => array("type"=>"bubble","body"=>array("type"=> "box","layout"=> "vertical","spacing"=> "sm","contents"=> array(
                                                                                    array("type"=> "text","text"=> "Produk tidak ada","size"=> "sm","weight"=> "bold","wrap"=> true))),
                                                                                'footer'=>array("type"=> "box","layout"=> "vertical","spacing"=> "sm","contents"=> array(
                                                                                    array(
                                                                                    "type"=> "button","action"=> array( "type"=> "message",  "label"=> "AKHIRI PESANAN",  "text"=> "selesai"),
                                                                                        "color"=> "#CC3E16",
                                                        ...
                                                        case 'delete':
                                                            ...
                                                            return "Menu yang telah dihapus";
                                                            break;
                                                        default:
                                                            $this->session->set('pilih',null);			
											...
											case 'pesan':
												switch($text)
												{
													case 'selesai':
														...
														// Antrian
														$antrian  = $db->query("select * from kerja where tanggal = '{$this->session->get('waktu')}'");
														$sum = $db->query("SELECT sum(harga) as harga,sum(laba) as laba from to_order where code ='{$this->session->get('code')}'");
														
														$result_sum = $sum->fetch_array();
														$total =$result_sum['harga'];
														$laba = $result_sum['laba'];
														$antrians = $antrian->fetch_object();

														$total_order = $db->query("insert into total_order(code,user_id,total,laba,tanggal,antrian) 
														values('{$this->session->get('code')}','{$this->profile->user_id}','{$total}',{$laba},'{$this->session->get('waktu')}','{$antrians->antrian}')");
