...
<div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0 text-dark">Bahan Penyusun {{ $data['produk']->name }}</h1>
                    </div>
                    <div class="col-sm-6">
...
​
                    ...
                    @component('components.card')
                            ...
                            <table class="table table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th><a href="{{ route('clearcookie')}}"><button class="btn btn-danger">CLEAR</button></a></th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <!-- MENGGUNAKAN LOOPING VUEJS -->
                                @if($data['material'])    
                                    @forelse($data['material'] as $a)
                                    <tr>
                                        <td>{{ $a['name'] }}</td>
                                        <td> {{ $a['qty_pakai'] }} </td>
                                        ...
                                        <td>
                                            ...
                                            <form action="{{ route('produk_material.delete',['id'=>$a['stok_id']])}}" method="post">
                                            @csrf
                                            @method('delete')
                                            <button 
                            ...
                            @slot('footer')
                            <form action="{{ route('storeProduk',['id'=>$data['produk']->id])}}" method="post">
                                @csrf
                                <div class="card-footer text-muted">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control" name="subcategori_id">
                                            @foreach($data['produk']->categori->subcategori as $a)
                    ...
                    @component('components.card')
                            ...
                            <table class="table table-hover">
                                ...
                                <tbody>
                                    ...
                                    @forelse($data['produk']->subcategori as $a)
                                    <tr>  
                                        ...
                                        <td>{{ $a->laba }}</td>
                                        <td>
                                            <form action="{{ route('produk_subcategori.edit',['id'=>$a->id])}}" method="post">
                                                @csrf
                                                @method('put')
                                                <button 
                                                    @click.prevent="removeCart(index)"    
                                            ...
                                            <form action="{{ route('produk_subcategori.delete',['id'=>$a->id])}}" method="post">
                                                @csrf
                                                @method('delete')
                                                <button 
                                                    @click.prevent="removeCart(index)"    
